...
# print(dataAoiPD)
all_tdid = []
with open('src/DataCleaning/AppFeaturesSorted.csv', 'r') as f:
    rows = csv.reader(f)
    row_index = 0
    for row in rows:
        row_index += 1
...
with open('src/DataCleaning/AoiFeaturesSorted.csv', 'r') as f:
    rows = csv.reader(f)
    row_index = 0
    for row in rows:
        row_index += 1
...
AoiFeaturesAppend = []
tdid_Aoi = []
with open('src/DataCleaning/AoiFeatures.csv', 'r') as f:
    rows = csv.reader(f)
    row_index = 0
    for row in rows:
        row_index += 1
...
dataAppPDFeaturesAppend.to_csv('src/DataCleaning/AoiFeaturesAppend.csv', encoding='utf-8_sig')
